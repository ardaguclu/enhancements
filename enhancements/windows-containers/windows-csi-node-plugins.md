...
* Windows pods can create, mount, read/write, and delete PVCs using supported StorageClasses and access modes
  (see Windows Storage Limitations section below for platform-level constraints).
...
* Achieving full feature parity with Linux storage. Windows does not support all volume modes, access modes, and
  volume features available on Linux. See the Windows Storage Limitations section below.
...
DaemonSets is the standard upstream pattern for multi-OS CSI deployments, as seen in the Azure File CSI driver
([csi-azurefile-node.yaml](https://github.com/kubernetes-sigs/azurefile-csi-driver/blob/master/deploy/csi-azurefile-node.yaml)
for Linux,
[csi-azurefile-node-windows-hostprocess.yaml](https://github.com/kubernetes-sigs/azurefile-csi-driver/blob/master/deploy/csi-azurefile-node-windows-hostprocess.yaml)
for Windows).
...
**Azure File CSI** (`file.csi.azure.com`) is deployed through the consolidated
...
(Azure File CSI driver patch with Windows paths and credentials). The shared infrastructure is reusable -- adding
Windows support to another csi-operator driver requires only the per-driver patch and configuration. Azure File CSI
uses the SMB protocol, which has native Windows support.

**VMware vSphere CSI** (`csi.vsphere.vmware.com`) is deployed through the standalone
...
upstream VMware vSphere CSI driver already has `csi-windows-support: "true"` enabled in its feature states ConfigMap
and handles Windows plus CSI proxy interactions internally.
...
N/A, Windows nodes are not supported on Hypershift.

#### Standalone Clusters

This enhancement is designed for standalone clusters where Windows nodes are supported.

#### Single-node Deployments or MicroShift

N/A, Windows nodes are not supported on single-node OpenShift or MicroShift.

#### OpenShift Kubernetes Engine

N/A, Windows node support is not included in OpenShift Kubernetes Engine.

### Risks and Mitigations

**Azure Credential Handling in HostProcess Pods**

The Azure File CSI Linux DaemonSet uses an `azure-inject-credentials` initContainer (a Linux binary from
`cluster-cloud-controller-manager-operator`). For the Windows DaemonSet, the Azure credentials secret is mounted
directly into the CSI driver container.

This introduces security considerations because the Windows DaemonSet uses HostProcess containers:
- On Windows, secret volumes write data to node local storage in clear text (unlike Linux tmpfs)
- HostProcess containers can access the host filesystem according to their Windows identity

Mitigations:
- Specify a least-privileged `runAsUserName` for the pod (e.g., `NT AUTHORITY\Local Service` or a dedicated
  service account with minimal permissions)
- The credentials secret should only be mounted into the CSI driver container, not the node-driver-registrar or
  liveness-probe sidecars
- Windows ACLs on the secret file path should restrict access to only the service account under which the driver runs

### Drawbacks

The main drawback is the increased complexity of managing both Linux and Windows DaemonSets within each CSI driver
operator. Each operator must maintain separate configurations for security contexts, filesystem paths, and
platform-specific settings. This increases the maintenance burden and requires testing on both platforms.

Another drawback is the introduction of Windows container image builds to the release pipeline. Building and
publishing multi-platform image manifests adds new infrastructure requirements and increases build complexity.

Additionally, the Azure File CSI driver will use different credential injection approaches for Linux and Windows. The
Linux DaemonSet uses an initContainer for credential injection, while the Windows DaemonSet mounts the credentials
secret directly. This divergence means the two implementations must be maintained separately.

Finally, Windows has inherent storage limitations compared to Linux (no block volumes, no fsGroup, no
mountPropagation). This means users expecting Linux feature parity will find that not all storage capabilities are
available on Windows nodes. Support teams will need to be familiar with these platform-specific limitations when
...
Per-driver configuration (example: Azure File CSI):
...
#### Windows DaemonSet Configuration Requirements

The Windows DaemonSet template must include the following configuration elements:

- **nodeSelector**: `kubernetes.io/os: windows` to ensure pods only schedule on Windows nodes
- **tolerations**: Match the Linux DaemonSet tolerations to ensure consistent scheduling behavior
- **priorityClassName**: `system-node-critical` to ensure CSI driver pods are not evicted
- **spec.os.name**: `windows` to explicitly declare the pod's operating system
- **hostNetwork**: `true` (required for HostProcess pods; enables health and metrics endpoints)
- **hostPath volumes**:
  - Kubelet plugin registration directory (`C:\var\lib\kubelet\plugins_registry`)
  - Kubelet plugins directory (`C:\var\lib\kubelet\plugins`)
- **CSI Proxy access**: HostProcess containers access CSI Proxy named pipes directly by path (e.g.,
  `\\.\pipe\csi-proxy-filesystem-v1` for Azure File CSI) without hostPath volume mounts
- **serviceAccount and RBAC**: Reuse the same ServiceAccount as the Linux DaemonSet; no Windows-specific RBAC changes required
- **securityContext**:
  - `windowsOptions.hostProcess: true` to enable host filesystem access and CSI proxy communication
  - `windowsOptions.runAsUserName`: Specify a least-privileged Windows account (e.g., `NT AUTHORITY\Local Service`
    or a dedicated service account with minimal permissions required for CSI operations)
...
executable, which is then packaged into a Windows runtime image using `buildah bud --platform windows/amd64`.

The choice between nanoserver (minimal runtime) and servercore (includes DNS and additional Windows features) will be
determined on a per-driver basis during implementation based on each driver's runtime dependencies.
...
This feature is targeted for OpenShift 5.1. The initial GA scope includes Azure File CSI and VMware vSphere CSI. Additional
...
If Windows CSI node driver DaemonSets were manually deployed, remove them before upgrading to avoid CSI registration
conflicts.
...
N/A. This enhancement does not introduce new API extensions (CRDs, admission webhooks, conversion webhooks, aggregated
API servers, or finalizers).
...
Prototype implementations have been completed for both the csi-operator (Azure File CSI) and the
...
An alternative for the Azure File CSI credential injection is to build a Windows version of the
