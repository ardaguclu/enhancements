...
Users specify plugin-specific configuration (details TBD). Whether the plugin name field will be dropped in favor of managed configuration is TBD.
...
// +unionMember
// +optional
// Deprecated: There is no logic listening this resource type, we plan to remove it in next release.
...
In Tech Preview v2, External type support is dropped in favor of managed plugin lifecycle.
Vault is introduced as the first managed KMS provider type.
The name field is deprecated as it's no longer used for manually managed static pods.
This field may be preserved if it provides value in the managed plugin lifecycle (details TBD).
...
// name specifies the KMS plugin name.
// Deprecated: This field is no longer used for External plugin management.
// May be preserved for managed plugin lifecycle if needed.
//
// +kubebuilder:validation:MaxLength=80
// +kubebuilder:validation:MinLength=1
// +required
